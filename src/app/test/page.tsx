...
import { testMode } from "@/lib/modes/test";
  ...
  return (
    <main className="flex flex-1 flex-col items-center gap-6">
      <div className="flex w-full max-w-md items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{testMode.name}</h1>
          <p className="text-sm text-slate-500">{testMode.description}</p>
        </div>
      ...
      {unlocked ? (
        <SessionRunner mode={testMode} words={words} />
      ) : (
        <div className="flex w-full max-w-md flex-col items-center gap-6 rounded-2xl bg-white p-8 text-center shadow-lg">
          <div className="text-6xl">🎧</div>
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Headphones recommended</h2>
            <p className="mt-1 text-sm text-slate-600">
              Words will be spoken aloud. Tap below to enable audio (required on iPhone).
            </p>
          </div>
          <button
            onClick={handleStart}
